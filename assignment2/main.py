...
personal_best_fitnesses = [evaluate_fitness(p, X_train, y_class_train, y_reg_train, inverse_CE, inverse_MSE, 2) for p in particles]  # Evaluate fitness for each particle
...
for iteration in range(MAX_ITERATIONS):
    for i in range(POPULATION_SIZE):
        ...
        current_fitness = evaluate_fitness(particles[i], X_train, y_class_train, y_reg_train, inverse_CE, inverse_MSE, 2)
...
# Calculate weights for test set using the global best position
feature_count = X.shape[1]  # Number of features in the dataset
...
# Classification predictions
predicted_prob = sigmoid(np.dot(X_test, W_class) + bias_class)
classification_ce = log_loss(y_class_test, predicted_prob)  # Compute loss for classification
classification_accuracy = accuracy(y_class_test, predicted_prob)

# Regression predictions on validation set
y_reg_pred_standardized = np.dot(X_test, W_reg) + bias_reg
y_reg_pred = scaler_y.inverse_transform(y_reg_pred_standardized.reshape(-1, 1)).flatten()  # Inverse transform to original scale
regression_mse_val = mean_squared_error(y_reg_test, y_reg_pred)
